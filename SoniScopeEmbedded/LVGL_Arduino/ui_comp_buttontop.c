...
lv_obj_remove_flag( cui_buttontop, LV_OBJ_FLAG_SCROLLABLE );    /// Flags
...
cui_button_top_icon = lv_image_create(cui_buttontop);
lv_image_set_src(cui_button_top_icon, &ui_img_call1_png);
...
lv_obj_remove_flag( cui_button_top_icon, LV_OBJ_FLAG_SCROLLABLE );    /// Flags

lv_obj_t *cui_line;
cui_line = lv_image_create(cui_buttontop);
lv_image_set_src(cui_line, &ui_img_btn_bg_1_png);
...
lv_obj_remove_flag( cui_line, LV_OBJ_FLAG_SCROLLABLE );    /// Flags

lv_obj_t ** children = lv_malloc(sizeof(lv_obj_t *) * _UI_COMP_BUTTONTOP_NUM);
