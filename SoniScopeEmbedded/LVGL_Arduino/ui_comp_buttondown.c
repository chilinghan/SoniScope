...
lv_obj_remove_flag( cui_buttondown, LV_OBJ_FLAG_SCROLLABLE );    /// Flags
...
cui_button_down_icon = lv_image_create(cui_buttondown);
...
lv_obj_remove_flag( cui_button_down_icon, LV_OBJ_FLAG_SCROLLABLE );    /// Flags

lv_obj_t *cui_line;
cui_line = lv_image_create(cui_buttondown);
lv_image_set_src(cui_line, &ui_img_btn_bg_3_png);
...
lv_obj_t ** children = lv_malloc(sizeof(lv_obj_t *) * _UI_COMP_BUTTONDOWN_NUM);
